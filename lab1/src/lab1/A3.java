...
class NodeA3 {
	...
	public NodeA3(NodeA3 father, int[][] state,int[][] GOAL_STATE,int direction, int zero_row, int zero_column,int m) {
		this.s=m;
...
public class A3 extends Myinit{
...
//	public static final int[][] WRONG_STATE = { { 0, 1, 2 }, { 3, 4, 5 }, { 7,6,8 } };
   ...
   this.addWindowListener(new WindowAdapter() {
    ...
    public void printResult(NodeA3 node){
        if(node.father == null) {
            return;
        }
...
// **************************************** 真正的主方法: ********************************************
		...
		PriorityQueue<NodeA3> pqueue = new PriorityQueue<>(new Comparator<NodeA3>() {
			public int compare(NodeA3 o1, NodeA3 o2) {
		...
		// 初始状态节点
		NodeA3 start = new NodeA3(null, START_STATE,GOAL_STATE,0, zeroIdx/m, zeroIdx%m,m);
		pqueue.add(start);
		...
		while (!pqueue.isEmpty()) {
			...
			if (node.statestring.equals(Arrays.deepToString(GOAL_STATE))) {
				...
				long endTime = System.currentTimeMillis();
              	...
              	dataField2.setText(dataField2.getText()+"A3用时： " + (endTime - startTime) + " ms");
              	dataField2.setText(dataField2.getText()+"  A3消耗内存： " + ( endMem-startMem ) / 1024 + " Kb");
...
//				continue;
			...
			for (int[] di : DIRECTIONS) {
				...
				NodeA3 newnode = new NodeA3(node, newstate, GOAL_STATE,direct,zero_r, zero_c,m);
