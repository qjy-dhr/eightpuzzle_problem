...
class NodeA2 {
	...
	public NodeA2(NodeA2 father, int[][] state,int[][] GOAL_STATE,int direction, int zero_row, int zero_column,int m) {
		this.s=m;
		...
		for (int i = 0; i < m; i++) {
			for (int j = 0; j < m; j++) {
				...
				this.distance += Math.sqrt((Math.pow((x-i),2))+(Math.pow((y-j),2)));
			}
		}
...
public class A2  extends Myinit{
...
//	public static final int[][] WRONG_STATE = { { 0, 1, 2 }, { 3, 4, 5 }, { 7,6,8 } };
   ...
   this.addWindowListener(new WindowAdapter() {
    ...
    public void printResult(NodeA2 node){
        if(node.father == null) {
            return;
        }
...
// **************************************** 真正的主方法: ********************************************
		...
		PriorityQueue<NodeA2> pqueue = new PriorityQueue<>(new Comparator<NodeA2>() {
			public int compare(NodeA2 o1, NodeA2 o2) {
		...
		// 初始状态节点
		NodeA2 start = new NodeA2(null, START_STATE,GOAL_STATE,0, zeroIdx/m, zeroIdx%m,m);
		pqueue.add(start);
		...
		while (!pqueue.isEmpty()) {
			...
			if (node.statestring.equals(Arrays.deepToString(GOAL_STATE))) {
				...
				long endTime = System.currentTimeMillis();
              	...
              	dataField2.setText(dataField2.getText()+"A2用时： " + (endTime - startTime) + " ms");
              	dataField2.setText(dataField2.getText()+"  A2消耗内存： " + ( endMem-startMem ) / 1024 + " Kb");
...
//				continue;
			...
			for (int[] di : DIRECTIONS) {
				...
				NodeA2 newnode = new NodeA2(node, newstate, GOAL_STATE,direct,zero_r, zero_c,m);
